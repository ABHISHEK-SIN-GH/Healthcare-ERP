...
import { deleteAdmin, getAllAdmin } from "../../utils/apis/AdminApi";
  ...
  return (
    <tr className="odd:bg-gray-50">
      ...
      <td className="whitespace-nowrap px-4 py-2 text-center">
        {(Permission.adminsPermission.edit).includes(userRole) ?
        <Link
          to={`/admin-panel/admins/edit/${uid}`}
          class="inline-block rounded bg-gray-400 px-4 py-2 text-xs font-medium text-white hover:bg-gray-700 me-2"
        >
          <i class="fa-solid fa-pen-to-square me-2"></i> Edit
        </Link> : '' }
        {(Permission.adminsPermission.delete).includes(userRole) ?
        <Link
          to="/admin-panel/admins"
          onClick={() => {
            deleteSelectedAdmin(uid);
          }}
...
export default function Admin() {
  ...
  const loadAllAdmins = async () => {
    try {
      ...
      console.log(admins);
      setAdminRecords(admins);
      setTotalNo(Math.ceil((admins?.length)/10));
      setFilteredRecords(admins?.slice((0+(pageNo-1)*10),10*pageNo));
      setLoader(false);
    } catch (error) {
      console.log(error);
      setLoader(false);
  ...
  const deleteSelectedAdmin = async (id) => {
    try {
      const response = await deleteAdmin(id, token);
  ...
  const searchQuery = () => {
    if(query.length>0){
      const filteredUser = adminRecords.filter((user)=>{
  ...
  const clearQuery = () => {
    setFilteredRecords(adminRecords);
    setFiltered(false);
    setQuery('');
  }
  ...
  return (
    <>
      <h1 className="text-2xl font-bold text-start mb-6">All Admins Records</h1>
      <div className="flex justify-between flex-col md:flex-row gap-6">
        ...
        {(Permission.adminsPermission.add).includes(userRole) ?
        <Link
          to={"/admin-panel/admins/add"}
              ...
              ? filteredRecords.map((user,index) => {
                  return (
                    <Record
                      ...
                      status={user.status}
                      userRole={userRole}
                      deleteSelectedAdmin={deleteSelectedAdmin}
                    />
                  );
                })
              : ""}
