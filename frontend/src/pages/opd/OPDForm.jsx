...
import { getOPDPatient, registerOPDPatient, updateOPDPatient } from "../../utils/apis/OPDPatientApi";
...
export default function OPDForm() {
  ...
  const [opdIpd,setOpdIpd] = useState('opd');
  ...
  const handleRegisterOPDPatient = async () => {
    const data = {
      fname,
      lname,
      phone,
    ...
    try {
      const response = await registerOPDPatient(data,token);
  ...
  const handleUpdateOPDPatient = async () => {
    const data = {
      fname,
      lname,
      phone,
    ...
    try {
      const response = await updateOPDPatient(id,data,token);
      console.log(response);
  ...
  const loadOPDPatient = async (id,token) => {
    try {
      const patient = await getOPDPatient(id,token);
  ...
  return (
    <>
      <h1 className='text-2xl font-bold text-start mb-6'>OPD Patient Registration</h1>
      <form className="mt-8 grid grid-cols-6 gap-6">
        ...
        <DropdownInput value={opdIpd} action={setOpdIpd} label={"OPD / IPD"} items={[{name:"Opd",value:"Opd"}]}/>
        ...
        <DropdownInput value={depart} action={setDepart} label={"Select Department"} items={departments}/>
        <DropdownInputID value={consultant} action={setConsultant} label={"Select Doctor"} items={allDoctors}/>
        ...
        {
          (id) ?
          <Button label={"update"} color={"white"} bgColor={"black"} action={handleUpdateOPDPatient}/> :
          <Button label={"submit"} color={"white"} bgColor={"black"} submit={printData} action={handleRegisterOPDPatient}/>
        }
