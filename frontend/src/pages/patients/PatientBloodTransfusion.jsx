...
import { getPatientBloodTransfusions, addPatientBloodTransfusions, updatePatientBloodTransfusions } from "../../utils/apis/PatientDetailsApi";
...
const bloodTransfusions = ["2024-04-01", "08:00", "120/80 mmHg", "70 bpm", "98%", "36.5°C", "90 mg/dL"]

const ThreadBox = ({Id,NoteId,Status,Name,Time,Date,Text,Img,BloodTransfusion,handleExpireNote}) => {
  return (
          ...
          <div class="relative flex flex-col leading-1.5 p-4 border-gray-200 bg-gray-100 rounded-e-xl rounded-es-xl dark:bg-gray-700 h-full sm:h-52 overflow-y-auto">
            ...
            }
            {
              ...
              <div class="text-sm font-normal text-gray-900 dark:text-white">
                <small><b>Parameter | Values</b></small> <hr className="my-1"/>
                <small><b>Blood Grp:</b> {BloodTransfusion.bloodGroup}</small> <hr className="my-1"/>
                <small><b>D-Time:</b> {BloodTransfusion.bloodDonationTime}</small> <hr className="my-1"/>
                <small><b>R-Time:</b> {BloodTransfusion.bloodRemovalTime}</small> <hr className="my-1"/>
              </div> : ''
            }
            {
              (Text) ? 
...
export default function PatientBloodTransfusion() {
  ...
  const [bloodTransfusions,setBloodTransfusions] = useState([]);
  const [bloodTransfusionsData,setBloodTransfusionsData] = useState([]);
  const [bloodTransfusionsId, setBloodTransfusionsId] = useState('');
  ...
  const handleSaveFile = async () => {
    ...
    try {
      ...
      if(response){
        ...
        setBloodTransfusions([bodyData,...bloodTransfusions]);
        if(bloodTransfusionsData.length==0){
          addBloodTransfusions(JSON.stringify([bodyData,...bloodTransfusions]));
        }else{
          updateBloodTransfusions(id,bloodTransfusionsData[0]._id,JSON.stringify([bodyData,...bloodTransfusions]));
        }
        hideFileModal();
  ...
  const [bloodGroup,setBloodGroup] = useState(allBloodGroups[0].value);
  const [bloodDonationTime,setDonationTime] = useState('');
  const [bloodRemovalTime,setRemovalTime] = useState('');
  const handleSaveForm = async () => {
    if(!bloodGroup,!bloodDonationTime,!bloodRemovalTime){
  ...
  const loadAllBloodTransfusions = async () => {
    try {
      const response = await getPatientBloodTransfusions(id,token);
      setBloodTransfusionsData(response);
      setBloodTransfusions(JSON.parse(response[0]?.patientBloodTransfusions));
      setBloodTransfusionsId(response[0]._id);
    } catch (error) {
      console.log(error);
    }
  }
  const addBloodTransfusions = async (data) => {
    const payload = {
      patientId:id,
      patientBloodTransfusions:data
    };
    try {
      const response = await addPatientBloodTransfusions(payload,token);
      loadAllBloodTransfusions();
    } catch (error) {
      console.log(error);
    }
  }
  const updateBloodTransfusions = async (uid,id,data) => {
    const payload = {
      patientId:uid,
      patientBloodTransfusions:data
    };
    try {
      const response = await updatePatientBloodTransfusions(id,payload,token);
    } catch (error) {
      console.log(error);
    }
  }
      ...
      buttons: [
        {
          label: 'Yes',
          onClick: () => {
            bloodTransfusions.forEach((bloodTransfusions)=>{
              if(bloodTransfusions.vid==id){
                bloodTransfusions.status = false;
              }
            })
            updateBloodTransfusions(uid,bloodTransfusionsId,JSON.stringify(bloodTransfusions));
            setBloodTransfusions([...bloodTransfusions]);
          }
        },
        {
          label: 'No',
          ...
          <div className="grid gap-2 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 2xl:grid-cols-6 pb-20">
            {
              (bloodTransfusions.length>0) ?
                bloodTransfusions.map((bloodTransfusion)=>{
                  return (
                    (bloodTransfusion.type=='form') ? <ThreadBox key={bloodTransfusion.vid} Id={id} Status={bloodTransfusion.status} NoteId={bloodTransfusion.vid} Name={bloodTransfusion.user} Time={bloodTransfusion.time} Date={bloodTransfusion.date} BloodTransfusion={bloodTransfusion} handleExpireNote={handleExpireNote}/> : 
                    (bloodTransfusion.type=='file') ? <ThreadBox key={bloodTransfusion.vid} Id={id} Status={bloodTransfusion.status} NoteId={bloodTransfusion.vid} Name={bloodTransfusion.user} Time={bloodTransfusion.time} Date={bloodTransfusion.date} Img={bloodTransfusion} handleExpireNote={handleExpireNote}/> :
                    <ThreadBox key={bloodTransfusion.vid} Id={id} Status={bloodTransfusion.status} NoteId={bloodTransfusion.vid} Name={bloodTransfusion.user} Time={bloodTransfusion.time} Date={bloodTransfusion.date} Text={bloodTransfusion} handleExpireNote={handleExpireNote}/>
                  )
                })
               :
...

      {
        ...
        <div class="z-10 text-black divide-y divide-gray-100 rounded-lg shadow w-52 absolute bottom-0 mb-16" ref={addBoxRef}>
          <ul class="p-2 text-sm bg-gray-600">
            <li>
              <select type="text" className="w-full" placeholder="Blood Group.." value={bloodGroup} onChange={(e)=>{setBloodGroup(e.target.value)}}>
                {
                  allBloodGroups.map((bloodGrp)=>{
                    return <option value={bloodGrp.value}>{bloodGrp.name}</option>;
                  })
                }
              </select>
            </li>
            <li>
              <input type="text" onFocus={(e)=>{e.target.type='time'}} className="w-full" placeholder="Donation Time.."  value={bloodDonationTime} onChange={(e)=>{setDonationTime(e.target.value)}}/>
            </li>
            <li>
              <input type="text" onFocus={(e)=>{e.target.type='time'}} className="w-full" placeholder="Removal Time.." value={bloodRemovalTime} onChange={(e)=>{setRemovalTime(e.target.value)}}/>
            </li>
            <li>
