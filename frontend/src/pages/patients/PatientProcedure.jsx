...
import { addPatientProcedures, getPatientProcedures, updatePatientProcedures } from "../../utils/apis/PatientDetailsApi";
...
const procedures = ["2024-04-01", "08:00", "120/80 mmHg", "70 bpm", "98%", "36.5°C", "90 mg/dL"]

const ThreadBox = ({Id,NoteId,Status,Name,Time,Date,Text,Img,Procedure,handleExpireNote}) => {
          ...
          <div class="relative flex flex-col leading-1.5 p-4 border-gray-200 bg-gray-100 rounded-e-xl rounded-es-xl dark:bg-gray-700 h-full sm:h-52 overflow-y-auto">
            ...
            }
            {
              ...
              <div class="text-sm font-normal text-gray-900 dark:text-white">
                ...
                <small><b>BP:</b> {Procedure.name}</small> <hr className="my-1"/>
              </div> : ''
            }
            {
              (Text) ? 
...
export default function PatientProcedure() {
  ...
  const handleSaveFile = async () => {
    ...
    try {
      ...
      if(response){
        ...
        setProcedures([bodyData,...procedures]);
        if(proceduresData.length==0){
          addProcedures(JSON.stringify([bodyData,...procedures]));
        }else{
          updateProcedures(id,proceduresData[0]._id,JSON.stringify([bodyData,...procedures]));
        }
        hideFileModal();
  ...
  const [name,setName] = useState('RBS');
  ...
  const loadAllProcedures = async () => {
    try {
      const response = await getPatientProcedures(id,token);
      console.log(response);
      if(response[0]){
        setProceduresData(response);
        setProcedures(JSON.parse(response[0]?.patientProcedures));
        setProcedureId(response[0]._id);
      }
    } catch (error) {
      console.log(error);
    }
  ...
  const addProcedures = async (data) => {
    const payload = {
      patientId:id,
      patientProcedures:data
    };
    try {
      const response = await addPatientProcedures(payload,token);
      loadAllProcedures();
    } catch (error) {
      console.log(error);
    }
  }
  const updateProcedures = async (uid,id,data) => {
    const payload = {
      patientId:uid,
      patientProcedures:data
    };
    try {
      const response = await updatePatientProcedures(id,payload,token);
    } catch (error) {
      console.log(error);
    }
  }
      ...
      buttons: [
        {
          ...
          onClick: () => {
            procedures.forEach((procedure)=>{
              if(procedure.vid==id){
                procedure.status = false;
              }
            })
            updateProcedures(uid,procedureId,JSON.stringify(procedures));
          ...
          <div className="grid gap-2 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 2xl:grid-cols-6 pb-20">
            {
              (procedures.length>0) ?
                procedures.map((procedure)=>{
                  return (
                    (procedure.type=='form') ? <ThreadBox key={procedure.vid} Id={id} Status={procedure.status} NoteId={procedure.vid} Name={procedure.user} Time={procedure.time} Date={procedure.date} Procedure={procedure} handleExpireNote={handleExpireNote}/> : 
                    (procedure.type=='file') ? <ThreadBox key={procedure.vid} Id={id} Status={procedure.status} NoteId={procedure.vid} Name={procedure.user} Time={procedure.time} Date={procedure.date} Img={procedure} handleExpireNote={handleExpireNote}/> :
                    <ThreadBox key={procedure.vid} Id={id} Status={procedure.status} NoteId={procedure.vid} Name={procedure.user} Time={procedure.time} Date={procedure.date} Text={procedure} handleExpireNote={handleExpireNote}/>
                  )
                })
               :
...

      {
        ...
        <div class="z-10 text-black divide-y divide-gray-100 rounded-lg shadow w-52 absolute bottom-0 mb-16" ref={addBoxRef}>
          <ul class="p-2 text-sm bg-gray-600">
            <li>
              <select className="w-full" placeholder="Procedures.." onChange={(e)=>{setName(e.target.value)}}>
                {
